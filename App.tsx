...
export default function App() {
  ...
  const scrollToBottom = (behavior: ScrollBehavior = 'smooth', delay = 100) => {
    setTimeout(() => { messagesEndRef.current?.scrollIntoView({ behavior, block: 'end' }); }, delay);
  };

  // EFEITO OTIMIZADO: Só salva/calcula se NÃO estiver streamando
  useEffect(() => {
    if (isLoadingHistory || status === AppStatus.STREAMING || status === AppStatus.THINKING) return; 
    ...
    scrollToBottom();
  }, [messages, activeModuleId, isLoadingHistory, status]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      for await (const chunk of stream) {
        ...
        // OTIMIZAÇÃO DE RENDERIZAÇÃO (Throttling)
        // Atualiza o estado apenas a cada ~40ms para evitar travamento do React
        const now = Date.now();
        if (now - lastUpdate > 40) {
            setMessages(prev => prev.map(m => m.id === botId ? { ...m, text: fullText } : m));
            // Scroll rápido sem delay para acompanhar a digitação
      ...
      // Salvar apenas no final do processo
      const updatedMessages = [...messages, userMsg, { id: botId, role: 'model', userName: 'Ainsten', text: fullText, timestamp: Date.now() }] as Message[];
      storageService.saveChat(activeModuleId, updatedMessages);
      const totalText = updatedMessages.reduce((acc, msg) => acc + msg.text, '');
      setContextCount(estimateTokens(totalText));
